...
namespace Game.UI
{
        ...
        void OnSelectSlot(UIWorkshopItemSlot itemSlot)
        {
            if(itemSlot.IsAssigned())
            {
                if(selectedSocket > -1)
                {
                    ...
                    {
                        Notifications.list.Add("please select a gem");
                        return;
                    }
                    if(!selectedSlot.IsAssigned())
                    {
                        Notifications.list.Add("please select equipment item to inlay");
                        return;
                    }
                    if(selectedSlot.data.HasGemWithType(((GemItem)itemSlot.data.data).bonusType))
                    {
                        Notifications.list.Add("item already inlayed with a gem of the same type");
                        return;
                    }
    ...
    #region Socket Slots
        ...
        public void OnRemove(int index)
        {
            ...
            {
                Notifications.list.Add("socket is locked");
                return;
            }
            if(!socketSlots[index].IsAssigned())
            {
                Notifications.list.Add("socket is already empty");
                return;
            }
    ...
    #region Unlock Socket
        ...
        public void OnUnlock()
        {
            ...
            {
                Notifications.list.Add("Please select a socket to unlock");
                return;
            }
            if(!selectedSlot.IsAssigned())
            {
            ...
            {
                Notifications.list.Add("socket isn't locked");
                return;
            }

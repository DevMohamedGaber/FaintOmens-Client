...
namespace Game.UI
{
    ...
    {
        // item
        public void Show(Item item, UIToolTipFrom from = UIToolTipFrom.None, int index = -1)
        {
            Set(from, index);
            ShowItem(item);
        ...
        {
            bool canUse = from == UIToolTipFrom.Inventory && (item.data is UsableItem);
        ...
        {
            if(from == UIToolTipFrom.Inventory)
            {

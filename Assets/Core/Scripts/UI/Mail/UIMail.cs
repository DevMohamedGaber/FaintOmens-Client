    ...
    public class UIMail : MonoBehaviour
    {
        ...
        void UpdateMailList()
        {
            ...
            if(player.own.mailBox.Count > 0)
            {
                for(int i = 0; i < player.own.mailBox.Count; i++)
                {
                    ...
                    {
                        mailBtn.lang.SetCode(0, Convert.ToInt32(mail.subject));
                    }
                    mailBtn.lang.SetPrefix(1, mail.senderName);//sender
                    mailBtn.lang.SetPrefix(2, "30d");
                    mailBtn.lang.Refresh();
                    //icon
                    if(mail.currency.gold > 0) mailBtn.image.sprite = UIManager.data.assets.currency[0];
                    else if(mail.currency.diamonds > 0) mailBtn.image.sprite = UIManager.data.assets.currency[1];
                    else if(mail.currency.b_diamonds > 0) mailBtn.image.sprite = UIManager.data.assets.currency[2];
                    else if(mail.items.Length > 0) mailBtn.image.sprite = mail.items[i].item.data.GetImage();
        ...
        public void OnCollectAll()
        {
            if(player.own.mailBox.Count > 0)
            {
                ...
                if(notEmpty)
                {
                    player.CmdCollectAllMails();
                }
                else
                {
                    Notifications.list.Add("All mails are empty.");
                }
            }
        }
        public void OnCollect()
        {
            if(current > -1 && player.own.mailBox.Count > current)
            {
                ...
                {
                    Notifications.list.Add("Already Recieved.");
                }
            }
        }
